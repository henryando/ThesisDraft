...
\begin{table}[t]
  ...
  \begin{tabular}{|c| c| c | c | c | c | c | c | c|}
    ...
    \hline
  \end{tabular}
  \caption{Fitted linewidths in \wn\ for all $Z_{1}Y_{n}$ excitation transitions in Sites 1, 2, 3, and 4, at 13, 32, 60, and 106 K. Peaks where a Gaussian distribution fit better than a Lorentzian based on the $R^{2}$ value of the fit are listed in parentheses. Peaks that were not visible are marked with an asterisk.}
...
\begin{figure}[b]
  ...
  \caption{Excitation peaks at $Z_1Y_1$ (left) and $Z_1Y_2$ (right) in Site 3. The intensity profile discussed in Equation \eqref{eq:1} is fitted to the $Z_{1}Y_{2}$ data.}
...
The form of the intensity $I$ as a function of the detuning $x$ is thus \begin{equation}\label{eq:1}
  I(x) = N\int_{x'}dx'\frac{1}{x'^{2}+(\Gamma_{3}/2)^{2}}\Big(
    a_{1} D(x - \Delta_{1}) e^{-x/\Gamma_{1}} + a_{2}D(\Delta_{2}-x)e^{x/\Gamma_{2}}
  \Big) ,
\end{equation}
where $D$ is the Heaviside step function, $N$ is a normalization prefactor, $\Gamma_{1}$-$\Gamma_{3}$ are linewidth parameters, and $\Delta_{1}$-$\Delta_{2}$ are 


\notetoself{In this section, I will discuss the findings of the linewidth analysis. I'm still working on it, but the goal is to fit both Gaussians and Lorentzians to the excitation peaks at $Z_{1}Y_{n}$, for all $n$, for all sites, for all temperatures. This will show when the lines are homogenously/inhomogeneously broadened, what the linewidths are, how they depend on temperature, and how things look at different sites. I'll also do a slightly more detailed discussion of the really bright, bimodal Site 3 peak. I will also include a table of the linewidths measured at each site by whichever functional form fit best, and the name of that functional form.}

\textbf{Potential for single-ion applications}: The observation of \erbium ions implanted in MgO is very exciting, because MgO has several desirable qualities as a potential host crystal. Magnesium and Oxygen both have stable isotopes with nuclear spin 0, so MgO can be made free of the nuclear spin problem that plagued YSO. This could enable substantially longer coherence times for \erbium spin qubits in MgO.




\notetoself{Another factor is our ability to stamp photonic crystals on top. Can we do this for MgO? Is there evidence we will be able to?}

\notetoself{The rest of this discussion depends a little bit on the linewidth findings. However, I'd like to discuss the strengths and weaknesses of the different sites. }
