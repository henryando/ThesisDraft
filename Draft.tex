...
% The good quantum numbers depend on the relative strength of spin-orbit interactions and electrostatic interactions. In the case of rare earth ions, the two types of interactions are comparably strong, and the appropriate basis of good quantum numbers is $J$ and $M$, the total angular momentum operator and the ``magnetic quantum number'' $M=-J, -J+1,...,J$. A basis of $JM$ eigenstates for these quantum numbers can be computed by starting from the standard $LS$ basis of eigenstates, then treating them to first order in perturbation theory. The resulting eigenstates are independent of $M$ (and therefore $2J+1$-fold degenerate), and consist of linear combinations of $LS$ eigenstates with $L+S=J$:
% \begin{equation}\label{eq:1}
% \Psi(nlJ) = \sum_{\tau L S}a_{\tau L S J}\ket{nl\tau L S J},
% \end{equation}
% where 
% \begin{equation}\label{eq:2}
% a_{\tau L S J} = \sum_{\tau'L'S'}\bra{nl\tau L S J }   H_{C} + H_{SO}   \ket{nl\tau'L'S'J'} \delta_{JJ'}.
% \end{equation}
% In the above equations, $H_{C}$ is the central field Hamiltonian, $H_{SO}$ is the spin-orbit Hamiltonian, $nl$ is the label for the radial part of the wavefunction, and $\tau$ is an additional ``seniority'' quantum number necessary for distinguishing states with the same values of $L$ and $S$. Further detail can be found in \cite{Liu2006}.
...
Our PLE setup is shown in Figure \notetoself{gotta make figure.}. A tunable diode laser is amplified by an erbium-doped fiber amplifier up to \notetoself{200 mW?}. The laser is then passed through a chopper. The 
